...
from certificates import accurate_lower_bound
...
'''
def brute_force(model, X, Y, k, n_th=1, verbose=0, maj_clss=None, lb_algo=accurate_lower_bound):
	...
	X = X[idx_correct]
	Y = Y[idx_correct]
	# removes with ALB instances for which there is no attack on k features that evades the model.
